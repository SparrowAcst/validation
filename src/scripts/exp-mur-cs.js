...
const getGradeAvg = d => {
	...
	if (d.m1 == d.m2) return avg([d.g1, d.g2])

	return avg( keys(d).filter( key => key.startsWith("g")).map( key => d[key]) )
...
// data = data.filter( d => d.m1 && d.m2 && d.m3 && d.m4 && d.m5 && d.m6)
	...
	data = data.map( d => {
		...
		d.gradeAvg = getGradeAvg(d)
		// d.ignoreByPresents = (Math.abs(d.absentCount - d.presentCount) < 2) ? "ignore" : ""
		d.ignoreByGrade = (d.mode == 'present' && d.gradeAvg <= 1) ? "ignore" : ""
	...
	let filtered_data = data.filter(d => !d.ignoreByGrade ) //d => !d.ignoreByPresents) // && !d.ignoreByGrade)
	let cm =  {
