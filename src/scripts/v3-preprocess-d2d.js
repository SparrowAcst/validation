...
let dataSources = [
	{
		xlsx: "./data/structures/V3-RRT-Validation-2024/SPECTRA/v3-rrt-d2d-iph-spectra.xlsx",
		json: "./data/v3/rrt/v3-rrt-d2d-iph.json"
	},
	{
		xlsx: "./data/structures/V3-RRT-Validation-2024/SPECTRA/v3-rrt-d2d-predicate-spectra.xlsx",
		json: "./data/v3/rrt/v3-rrt-d2d-predicate.json"
	}
]



const RES_XLSX = "./data/structures/V3-RRT-Validation-2024/SPECTRA/v3-rrt-d2d-processed.xlsx"
...
const getDevice = name => {
	let a = name.split("-")
	let res = (a[3] == "iph") ? `Device #${a[4].replace("d", "")}` : "Predicate"
...
const run = async () => {
	...
	await saveXLSX( 
		sortBy(
			sortBy( result.filter( r => r.id), r => r.id), 
			r => r.type
		), 
		RES_XLSX
	)
