 ...
 */
const call = (fileLocation, methodName, params, RBinariesLocation) => {
	let output = 0;
	...
	const source = (fileLocation) ? `source('${fileLocation}')` : ""
	const statement = `${methodName}(params = '${JSON.stringify(params).replace(/"/g,'\\"')}')`
	const script = `${source}; print(${statement})`
	output = executeRCommand(script, RBinariesLocation);
 ...
 */
const callAsync = (fileLocation, methodName, params, RBinariesLocation) => {
	return new Promise(function(resolve, reject) {
		...
		const statement = `${methodName}(params = '${JSON.stringify(params).replace(/"/g,'\\"')}')`
		const script = `source('${fileLocation}'); print(${statement})`

		executeRCommandAsync(script, RBinariesLocation).then((res) => {
			resolve(res);
		}).catch((error) => {
			reject(`${error}`);
		});
